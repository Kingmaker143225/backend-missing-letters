...
# Durations (must match audio)
COUNTDOWN_NUMBERS = [5, 4, 3, 2, 1]
COUNT_DURATION = 1.0       # each number shown for 1 second
ANSWER_DURATION = 2.0      # answer shown for 2 seconds
TOTAL_VIDEO_DURATION = len(COUNTDOWN_NUMBERS) * COUNT_DURATION + ANSWER_DURATION
...
def remove_white_background(image):
    ...
    for r, g, b, a in pixels:
        ...
        if (
            ...
            abs(r - g) < 10 and abs(g - b) < 10 and r > 200
        ):
...
def get_bounce_offset(t):
    ...
    """
    return int(12 * math.sin(t * 2 * math.pi * 1.5))
...
def create_frame(
    ...
    for x in range(20, WIDTH, 40):
        for y in range(20, HEIGHT, 40):
            draw.ellipse((x, y, x + 6, y + 6), fill=DOT_COLOR)
    ...
    if not show_answer and count is not None:
        draw_center_text(draw, str(count), 455, COUNT_FONT, ORANGE)
        draw_center_text(draw, "Find the missing letter", 550, SMALL_FONT, "#555")
...
async def generate_video(
    ...
    # =====================================================
    # AUDIO (exactly matches visual timings)
    # =====================================================
    ...
    # Countdown clips with bouncing image
    elapsed = 0.0
    ...
    for count in COUNTDOWN_NUMBERS:
        ...
        frame_path = os.path.join(
            OUTPUT_DIR,
            f"{unique_id}_count_{count}.png"
        )
        frame.save(frame_path)

        clip = ImageClip(frame_path).with_duration(COUNT_DURATION)
        clips.append(clip)

        elapsed += COUNT_DURATION
    ...
    answer_frame_path = os.path.join(
        OUTPUT_DIR,
        f"{unique_id}_answer.png"
    )
    answer_frame.save(answer_frame_path)

    clips.append(
        ImageClip(answer_frame_path).with_duration(ANSWER_DURATION)
    )
